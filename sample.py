...
def sample():
	...
	with tf.Session() as sess:
		...
		state2 = list(sess.run(model.C2[test_state_num]))
		print(state)
		#feed_predict[model.decorder_initial_state] = [(state, state2)]
		feed_predict[model.decorder_initial_state[0]] = [state]
		feed_predict[model.decorder_initial_state[1]] = [state]
