...
def train():
	...
	epochs = 1000
	batch_size = 1
	state_save_point = 2000
	current_save_point = 200
	...
	for _i in range(int(len(args)/2)):
		...
		if arg == '-r':
			...
			state_save_point = int(value) - 1
		elif arg == '-s':
			current_save_point = int(value) - 1
		

	print(restore, epochs, batch_size, state_save_point, current_save_point)
	...
	em_de_tran = np.transpose(em_encoded_sentences_for_decoder, (1,0,2))
	id_en_tran = np.transpose(id_encoded_sentences)	
		...
		with tf.device("/gpu:0"):	
			...
			for e in range(epochs):
				...
				for i in range(int(total_size/batch_size) + 1):
					...
					#******************************************************************************]
					if e % state_save_point == 0  and e != 0:
						print(encoder_states)
						print(type(encoder_states))
						for r in range(end-start):
							#--> never use like this : model.C[start + r] = encoder_states[r]
							sess.run(model.C[start + r].assign(encoder_states[0][r]))
							sess.run(model.C2[start + r].assign(encoder_states[1][r]))
					#******************************************************************************
					...
					for i in range(seq_length):
						feed_decoder_input[model.decoder_input[i]] = em_de_tran[i][start:end]
						feed_decoder_target[model.decoder_target[i]] = id_en_tran[i][start:end]
				...
				if e % 10 == 0:
					print("Take", str((dt.datetime.now() - start_time).seconds), "seconds for 1 cycles, at epoch : ",  e)
				
			
				if e % current_save_point == 0 and  e != 0:
