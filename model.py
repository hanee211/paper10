...
class Model():
	def __init__(self, params, training=True):
		...
		self.C2 = [tf.Variable(tf.ones([rnn_size])) for _ in range(total_size)]
		...
		for _ in range(num_layers):
			...
			_d_cell = tf.contrib.rnn.GRUCell(num_units=rnn_size)
			...
			cells.append(_cell)
			d_cells.append(_d_cell)
		
		encoder_cell = tf.contrib.rnn.MultiRNNCell(cells, state_is_tuple=True)
		decoder_cell = tf.contrib.rnn.MultiRNNCell(d_cells, state_is_tuple=True)
		...
		self.decorder_initial_state = (tf.placeholder(dtype=tf.float32, shape=[None, rnn_size]) , tf.placeholder(dtype=tf.float32, shape=[None, rnn_size]))
